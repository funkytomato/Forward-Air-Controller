...
namespace Missiles
{
    public class MissileSpawner : MonoBehaviour
    {
        ...
        void SpawnMisile()
        {
            ...
            if (number > 0)
            {
                if (launchExplotion) { Instantiate(launchExplotion, launchPoint.transform.position, launchPoint.transform.rotation); }

                newMissile = Instantiate(missile, launchPoint.transform.position, launchPoint.transform.rotation);
                //if ricochet
